...
"This file is the backbone of MCTS, which is a general-purpose algorithm for making decisions in a given domain by searching a tree of possible actions."

"""This MCTS code is of high quality and can be used in a variety of domains."""
...
class MCTS_Searcher:
    ...
    def do_rollout(self, root_node: MCTS_Node, rollout_id: int):     
        "Four steps of MCTS: Select, Expand, Simulate, Backpropagate"
        ...
        if self.terminate:
            return self.parent2children[leaf][-1]
        ...
        self._backpropagate(path_1 + path_2)       # path_1 + path_2 is the path from first unexplored node to terminal node
        try:
            return path_2[-1]    
        except:
            return path_1[-1]
    ...
    def _expand(self, node: MCTS_Node, rollout_id: int):
        ...
        if node.is_terminal():            # terminal node is non-expandable (which also means the simulation has been completed)
            self.explored_nodes.add(node)
            return  # terminal node is non-expandable

        self.parent2children[node], self.terminate = node.find_children(rollout_id)   # find_children not implemented

    def _simulate(self, node: MCTS_Node, rollout_id: int) -> List[MCTS_Node]:
        "Returns the reward for a random simulation (to completion) of `node`"
        "Note that the reward is calculated at the terminal node, and one node only can be expanded once"
        path = []
        cur_node = node
        while True:   
            ...
            if cur_node not in self.parent2children.keys():
                self.parent2children[cur_node], self.terminate = cur_node.find_children(rollout_id)
            
            if self.terminate:
                if self.parent2children[cur_node]:
                    path.append(self.parent2children[cur_node][-1])
                return path

            # guard against empty children
            children = self.parent2children[cur_node]
            if not children:
                self.explored_nodes.add(cur_node)
                return path

            cur_node = random.choice(children)
    ...
    def _uct_select(self, node: MCTS_Node, rollout_id: int, epsilon: float = 0.1):
        "Select a child of node using an epsilon-greedy strategy"

        # All children of the node should already be expanded (judge process)
        assert all(n in self.explored_nodes for n in self.parent2children[node])

        if random.random() < epsilon:  # Explore: select a random child
            return random.choice(self.parent2children[node])
        else:  # Exploit: select the child with the highest UCT score
            return max(
